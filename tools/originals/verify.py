...
def static_inventory(root, lock):
    """Preserve the two exact legacy JSON routes; no arbitrary file copying."""
    rows = lock.get('staticFiles', [])
    validate_rows(rows)
    if len(rows) > 2 or any(row['path'] not in {'archive-routing.json', 'releases/index.json'} for row in rows):
        raise ValueError('Unsupported preserved static route')
    for row in rows:
        path = root / 'static' / row['path']
        if not path.is_file() or path.is_symlink() or path.stat().st_size != row['bytes'] or digest(path) != row['sha256']:
            raise ValueError('Preserved static route bytes changed')
    return rows
...
def locked_inputs(root=ROOT):
    ...
    derived.extend(static_inventory(root, lock))
    derived.extend([
